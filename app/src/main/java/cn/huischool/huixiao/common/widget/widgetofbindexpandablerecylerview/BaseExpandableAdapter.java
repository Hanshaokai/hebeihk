...
public abstract class BaseExpandableAdapter extends RecyclerView.Adapter implements
        AbstractExpandableAdapterItem.ParentListItemExpandCollapseListener {
    ...
    protected BaseExpandableAdapter(List data) {
        ...
        this.mDataList = null;
        this.mDataList = data;
        LogUtil.d("=========初始项目个数" + mDataList.size()
        );
        checkDefaultExpand();
...
*/
     ...
     */
    private void collapseParentListItem(ExpandableListItem expandableListItem, int parentIndex,
                                        boolean collapseTriggeredByListItemClick) {//折叠即 去除三级列表数据
        if (expandableListItem.isExpanded()) {
...
*/
   ...
   /* @NonNull
    ...
    protected void expandParentListItem(ExpandableListItem expandableListItem, int parentIndex, boolean expansionTriggeredByListItemClick, boolean isExpandAllChildren) {
        if (!expandableListItem.isExpanded()) {//加入三级列表中的数据
            List<?> childItemList = expandableListItem.getChildItemList();
            if (childItemList != null && !childItemList.isEmpty()) {
