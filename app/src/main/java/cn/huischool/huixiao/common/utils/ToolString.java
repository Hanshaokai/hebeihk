...
public class ToolString {
    ...
    /*
    * 强中文转换成get可传输的格式*/
    public static String getStringOfEncoder(String s) {

        String string = "";
        try {
            string = URLEncoder.encode(URLEncoder.encode(s,
                    "UTF-8"), "UTF-8");
    ...
    /*
    * 得到数组中某itme的位置*/
    public static int getSelectionPosition(String[] s, String item) {
        int position = 0;
        for (int i = 0; i < s.length; i++) {
            if (s[i].equals(item)) {
                position = i;
                LogUtil.d(s[i]+"==="+"item"+item);
            }
